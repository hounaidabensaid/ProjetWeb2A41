...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    try {
        if ($action === 'confirm') {
            ...
            $stmt = $pdo->prepare("SELECT nom, tel FROM user WHERE id = ?");
            $stmt->execute([$reservation['id_user']]);
            $user = $stmt->fetch();
            if ($user && !empty($user['tel'])) {
                $controller->sendConfirmationSMS($user['tel'], $user['nom'], $reservation['date_début'], $reservation['date_fin']);
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($statut === 'confirmée') {
            ...
            if ($user && !empty($user['tel'])) {
                $controller->sendConfirmationSMS($user['tel'], $user['nom'], $date_debut, $date_fin);
            } else {
