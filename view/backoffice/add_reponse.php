...
$reclamation_id = isset($_GET['reclamation_id']) ? (int)$_GET['reclamation_id'] : null;
$admin_id = null; // Will be set to 1 in ReponseController

// Fetch the reclamation
$reclamationController = new ReclamationController();
$reclamation = null;
if ($reclamation_id) {
    $reclamation = $reclamationController->getReclamationById($reclamation_id);
    if (!$reclamation) {
        die("Erreur: Réclamation non trouvée.");
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($isValid) {
        try {
            ...
            // Update reclamation status to 'traitee'
            $reclamationController->updateReclamationStatus($reclamation_id, 'traitee');
            ...
            try {
                ...
                // Content
                $mail->isHTML(true);
                $mail->Subject = 'Réponse à votre réclamation #' . $reclamation['id'];
                $mail->Body = "
            ...
            header("Location: view_reponse.php?success=Réponse enregistrée et email envoyé avec succès.");
            exit();
        } catch (Exception $e) {
            $validationErrors[] = "Erreur: " . $e->getMessage();
        }
    }
}
include 'header.php';
...
<div class="form-container">
    <h3>Répondre à la Réclamation #<?php echo htmlspecialchars($reclamation['id'] ?? ''); ?></h3>
    ...
    <form id="responseForm" action="" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <label>Sujet:</label>
            <input type="text" value="<?php echo htmlspecialchars($reclamation['sujet'] ?? ''); ?>" class="disabled-input" disabled>
        </div>

        <div class="form-group">
            <label>Email:</label>
            <input type="text" value="<?php echo htmlspecialchars($reclamation['email'] ?? ''); ?>" class="disabled-input" disabled>
        </div>
