...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Traitement de l'image (facultatif, selon si fichier est fourni)
        $image = null;
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            $targetDir = "/uploads/voitures/";
            if (!is_dir($targetDir)) {
        ...
        $voiture = new Voiture($id, $matricule, $marque, $modele, $couleur, $nb_place, $statut, $image);
...
<form method="POST" id="voiture-form" enctype="multipart/form-data" novalidate>
    ...
    <?php if ($voitureToEdit && !empty($voitureToEdit->getImage())): ?>
        <div class="mt-2">
            <img src="uploads/voitures/<?= htmlspecialchars($voitureToEdit->getImage()) ?>" alt="Image actuelle" style="max-width: 100%; height: auto; border: 1px solid #ccc; border-radius: 4px;">
        </div>
    <?php endif; ?>
</div>
    ...
    <a href="http://localhost/ttttttesttttt/view/BackOffice/index_voiture.php?edit=<?= $voiture->getId() ?>" class="btn btn-sm btn-outline-primary me-1" title="Modifier">
        <i class="fas fa-edit"></i>
    </a>

    <!-- Bouton Supprimer avec confirmation -->
    <a href="http://localhost/ttttttesttttt/view/BackOffice/index_voiture.php?delete=<?= $voiture->getId() ?>" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette voiture ?')" class="btn btn-sm btn-outline-danger" title="Supprimer">
	...
	<script>
    const validMarques = ['Toyota', 'Peugeot', 'Renault', 'Volkswagen', 'Fiat','Audi']; // exemple
