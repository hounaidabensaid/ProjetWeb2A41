...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $reclamationId = $controller->addReclamation($reclamation);

        // Return success response as JSON
        echo json_encode([
            'success' => true,
            'message' => "Réclamation envoyée avec succès ! Vous recevrez une confirmation par email sous 24h. ID: $reclamationId"
        ]);
        exit;
